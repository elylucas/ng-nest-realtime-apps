...
will separate areas of concern.
...
websocket connection. Its purpose is to keep track of the user's `clientId`,
which is randomly generated each time a client makes a new connection. In your
applications, you will want a way to associate a `clientId` with a user, as you
will need the `clientId` to send individual messages to users. Our chat app uses
a plain JavaScript object where the user's name will be the key and the
`clientId` the value.

The gateway will call the disconnect method when a websocket loses its
...
`@SubscribeMessage` decorators in the last lab. One subtle difference, though,
...
will only be responsible for messages that get passed to this namespace.

The `handleIdentify` method will be called by the client immediately after
making a connection. We grab the `clientId` off the socket and pass it to the
service to keep track of the user.
...
On the client side, the Angular `ChatService` will be responsible for setting up
...
We could return each piece of data as its own observable, which could get
...
user enters their name and hits the connect button in the UI.
...
with the connection. After identifying, we set the `connected$` observable to
...
chat room without having to fill out their name again.
...
If not, we set the user in local storage, update the `user$` observable with the
user name, and initiate a socket connection.
...
methods are bound to buttons in the template, and the existing `message` and
...
At the top, update the `ng-container` method to set its `ngIf` statement to bind
to `chatAppData`. We'll use the `async` pipe to subscribe to the observable and
alias its return value to `data`, which we will use throughout the rest of the
template.
...
When you visit the app, you should be able to login/connect and see your name in
